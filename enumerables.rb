...
def spiciest_foods(spicy_foods)
  spicyFoods = spicy_foods.select { |food| food[:heat_level] > 5 }
end
...
def print_spiciest_foods(spicy_foods)
  spicyFoods = spiciest_foods(spicy_foods);
  print_spicy_foods(spicyFoods)
end
# given an array of spicy foods, return an integer representing 
# the average heat level of all the spicy foods in the array
def average_heat_level(spicy_foods)
  heatLevelTotal = 0
  spicy_foods.each {|food| heatLevelTotal += food[:heat_level]}
  heatLevelTotal / spicy_foods.length
end
